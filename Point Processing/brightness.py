...
def brightness(input, value):
    return np.clip(input + value, 0, 255)
...
def plot(pics, value, ope='Brightness'):
...
def main():
    ...
    parse.add_argument('-v', dest='value', type=float, default = 60., help='value for processing')
    ...
    if args.value:
        ...
        cv2.imwrite(output1, brightness(input, -abs(args.value)))
        output2 = args.output.split('.')[0] + '2.' + args.output.split('.')[1]
        cv2.imwrite(output2, brightness(input, abs(args.value)))
