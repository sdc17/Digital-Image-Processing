...
def contrast(input, value):
    return np.clip(value * (input - 127.) + 127. , 0, 255)
...
def plot(pics, value, ope='Contrast'):
...
def main():
    ...
    if args.value < 0.:
        print("Contrast value should be non-negative")
        return

    if args.value:
        output1 = args.output.split('.')[0] + '1.' + args.output.split('.')[1]
        cv2.imwrite(output1, contrast(input,  1. / args.value))
        output2 = args.output.split('.')[0] + '2.' + args.output.split('.')[1]
        cv2.imwrite(output2, contrast(input, args.value))
